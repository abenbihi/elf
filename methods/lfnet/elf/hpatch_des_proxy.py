...
parser.add_argument('--kp_dir_id', type=str, default='1', 
        help='dir that stores lift kp res/lfnet/"$kp_dir_id"/des')
...
kp_dir = os.path.join('res/lfnet/%s'%args.kp_dir_id)

# setup output dir
res_dir = 'res/lfnet/%s'%args.trials
if not os.path.exists(res_dir):
    os.makedirs(res_dir)

# write human readable logs
f = open(os.path.join(res_dir, 'log.txt'), 'w')
f.write('method: lfnet\n')
f.write('data: %s\n'%cst.DATA)
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            for scene_name in cst.SCENE_LIST:
                ...
                kp0 = []
                for pt in pts0.T:
                    kp = cv2.KeyPoint(x=pt[0],y=pt[1], _size=1,
