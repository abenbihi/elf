"""
Compute perfs on my kp with some mtf descriptor on webcam img
"""
...
parser.add_argument('--feat_name', type=str, default='pool3', help='feat_name')
args = parser.parse_args()

new_size = (args.w, args.h)
# TODO
kp_dir = 'res/tilde/%s/'%args.trials
print(kp_dir)
norm = 'L2'


# setup output dir
res_dir = os.path.join('res/tilde/', args.trials)
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            for scene_name in cst.SCENE_LIST:
                ...
                pts0 = np.loadtxt(os.path.join(kp_dir, scene_name, 
                    'wo/%s.txt'%root_fn))[1:,:].T
                ...
                for img_name in img_list[1:]:
                    ...
                    pts1 = np.loadtxt(os.path.join(kp_dir, scene_name, 
                        'wo/%s.txt'%root_fn))[1:,:].T
