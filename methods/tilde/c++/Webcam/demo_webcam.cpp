...
#define DATA_DIR "datasets/WebcamRelease/"
#define W 640
#define H 480
#define DEBUG 0
#define SCENE_NUM 6
...
int main(int argc,char** argv)
{
  ...
  // load scene list
  std::string scene_list_fn = "meta/list/img_webcam.txt";
  ...
  std::string res_fn = "";
    ...
    for (int i=0; i<SCENE_NUM; i++){
      ...
      std::string input_directory = std::string(DATA_DIR) + scene_name + "/test/image_color/";
      ...
      if ((dir=opendir(input_directory.c_str())) != NULL){ 
        while ( (ent=readdir(dir)) != NULL){
          ...
          std::cout << "img: " << ent->d_name << std::endl;

          std::string img_fn = std::string(ent->d_name);
          std::string img_root_fn = img_fn.substr(0, img_fn.find(".png"));
          img_fn = input_directory + img_fn;
          ...
          pathFilter = std::string(FILTER_DIR) + scene_name + ".txt";
          ...
          //pathFilter = std::string(FILTER_DIR) + scene_name + "24.txt";
