"""
Derived from official superpoint main code.
Online computing of matching score i.e. computes kp and des on the run.
kp: superpoint
des: superpoint
"""
import argparse
...
for scene_name in cst.SCENE_LIST:
    ...
    # get img 
    img0_fn = os.path.join(cst.DATA_DIR, scene_name,'%d.%s'%(1, cst.IMG_EXT))
    img0 = vs.read_image(img0_fn, (opt.H, opt.W))

    # detect and describe
    pts0, des0, heatmap0 = fe.run(img0)
    ...
    # draw kp
    img0 = cv2.cvtColor(cv2.imread(img0_fn), cv2.COLOR_BGR2GRAY)
    if opt.resize:
    ...
    for img_key in range(2, cst.MAX_IMG_NUM+1):
        ...
        img1 = vs.read_image(img1_fn, (opt.H, opt.W))
        
        # Get points and descriptors.
        pts1, des1, heatmap1 = fe.run(img1)
        ...
        # draw kp
        img1 = cv2.cvtColor(cv2.imread(img1_fn), cv2.COLOR_BGR2GRAY)
        if opt.resize==1:
            # correct H for new size
        ...
        (ms, N1, N2, M_len, M_d_len, inter) = bench_tools.ms(new_size, H,
                kp0, kp1, des0, des1, cst.THRESH_OVERLAP, cst.THRESH_DESC)
