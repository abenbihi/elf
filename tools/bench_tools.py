...
def ms2(shape, H, kp1, kp2, des1, des2, thresh_overlap, thresh_desc, norm='L2'):
    """
    Matching score. Run greedy bipartite matching only for kp and approximate
    knn using flann (faster than greedy bipartite matching).
    Difference with ms: performs less than ms with a difference of 10^-2.
    Args:
        shape: img shape (x,y):(j,i)
        H: ground truth H (such that img2 = H(img1))
        kp1: list of cv.keypoints in img1
    ...
    if denom==0:
        ...
        if norm=='L2':
            ...
            for i,(m,n) in enumerate(matches):
                if m.distance < 0.8*n.distance: # Lowe's ratio
                    if m.distance < thresh_desc: # matching condition
                        good.append(m)
        elif norm=='hamming':
        ...
        M_d = [(m.queryIdx, m.trainIdx) for m in good ]
